# -*- coding: utf-8 -*-
"""
Llama-3 + TFT MegaCap5 full-year (h=4) multi-horizon model.
Uses quarterly Llama-3 8B sentiment features as additional inputs.
Output format exactly matches the FinBERT baseline for direct comparison.
"""
...
LLAMA3_CSV = r"C:\ThesisResearch\finbert_tft\data\processed_sentiment\megacap5_quarterly_sentiment_llama3.csv"
OUT_DIR    = r"C:\ThesisResearch\finbert_tft\results\llama3_TFT_MegaCap5_h4_L4"
...
ATTN_HEADS      = 4
DROPOUT         = 0.15
MAX_ENCODER_LEN = 4     # 12 quarters of historical context
...
def _load_all(data_dir: str, llama3_csv: str) -> pd.DataFrame:
    ...
    # -----------------------------------------------------------------
    # INTEGRATING LLAMA-3 METRICS
    # -----------------------------------------------------------------
    if not os.path.exists(llama3_csv):
        raise RuntimeError(f"Llama-3 quarterly file not found: {llama3_csv}")

    sent = pd.read_csv(llama3_csv)
    required_sent = {"ticker", "Year_Quarter", "sent_net_mean", "sent_fli_count"}
    missing_sent = required_sent - set(sent.columns)
    if missing_sent:
        raise ValueError(f"Llama-3 CSV missing required columns: {sorted(missing_sent)}")
    ...
    # Shift logic to prevent lookahead bias, and ffill for known future reals
...
def choose_known_future_cols(df: pd.DataFrame):
    ...
    # Llama-3 integration - dual-role forward fill
    sent_cols = ["sent_net_mean_ffill", "sent_fli_count_ffill"]
    known_reals.extend([c for c in sent_cols if c in df.columns])
...
def build_feature_lists(df: pd.DataFrame):
    ...
    # Llama-3 integration - observed past sentiment
    base_sent = ["sent_net_mean", "sent_fli_count"]
    unknown_reals.extend([c for c in base_sent if c in df.columns])
...
def _extract_and_save_per_epoch_metrics(trainer, out_dir) -> pd.DataFrame:
    ...
    try:
        ...
        elif 'train_loss' in per_epoch_df.columns: 
             pass

        lr_cols = [c for c in per_epoch_df.columns if c.startswith('lr-')]
        if lr_cols:
            name_mapping[lr_cols[0]] = 'learning_rate' 
...
def visualize_tft_results(metrics_df, tft_model, dataset, results_df, out_dir):
    ...
    if not metrics_df.empty:
        ...
        if 'learning_rate' in metrics_df.columns:
             ...
             if not lr_log.empty:
                  ...
                  plt.title("TFT Learning Rate Schedule", fontsize=14)
                  plt.xlabel("Epoch", fontsize=12)
    ...
    if os.path.exists(interpret_enc_path):
        ...
        if len(importance_means) == len(encoder_variables):
            ...
            plt.title("TFT Variable Selection Importance (Encoder)", fontsize=14)
    ...
    if os.path.exists(interpret_att_path):
        ...
        if attention_array.size > 0:
            if len(attention_array.shape) == 4:
                ...
                plt.title("TFT Masked Multi-Head Attention", fontsize=14)
                plt.xlabel("Historical Quarters", fontsize=12)
...
def main():
    ...
    df = _load_all(DATA_DIR, LLAMA3_CSV)
